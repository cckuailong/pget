...
func (p *Pget) Checking() error {
	...
	if ! p.quiet{
		_,_ = fmt.Fprintf(os.Stdout, "Checking now %s\n", p.URL)
	}
	go p.CheckMirrors(ctx, p.URL, ch)
...
func (p *Pget) Download() error {
	...
	if err := p.Utils.ProgressBar(ctx, p.quiet); err != nil {
		return err
	}
...
func (p Pget) Assignment(grp *errgroup.Group, procs, split uint) {
	...
	for i := uint(0); i < procs; i++ {
		...
		if totalActiveProcs%assignment == 0 {
			...
			if lasturl != url {
				if !p.quiet {
					_, _ = fmt.Fprintf(os.Stdout, "Download start from %s\n", url)
				}
				lasturl = url
			}
		}
